...
const createSVG = <T extends SVGElement>(
  tag: string,
...
export class CursorSimulator {
  ...
  constructor(options: CursorOptions = {}) {
    this.options = {
      ...
      color: options.color || "var(--superwizard-brand-primary, #1D7BA7)",
  ...
  public initialize(): void {
    ...
    this.svgElement = createSVG(
      "svg",
      {
        id: "superwizard-cursor-svg",
        viewBox: "0 0 349 385",
        fill: "none",
      },
      {
        position: "absolute",
        width: `${this.options.size * 1.32}px`,
        height: `${this.options.size * 1.452}px`,
        transform: "translate(0, 0)",
        transformOrigin: "2.2px 2.2px",
        opacity: "0",
    ...
    const filter = createSVG<SVGFilterElement>("filter", {
      id: "cursor-shadow",
      x: "-50%",
      y: "-50%",
      width: "200%",
    ...
    filter.appendChild(
      createSVG<SVGFEDropShadowElement>("feDropShadow", {
        dx: "2",
        dy: "2",
        stdDeviation: "3",
    ...
    this.cursorCircle = createSVG<SVGPathElement>("path", {
      d: "M10.765 39.684C6.524 19.414 28.223 3.648 46.19 13.946l280.329 160.66c18.26 10.465 15.166 37.691-4.977 43.791l-127.896 38.736a4.006 4.006 0 0 0-2.152 1.584l-71.318 105.216c-11.889 17.541-39.018 12.192-43.358-8.55l-66.053-315.7Z",
      fill: "#1D7BA7",
      stroke: "#fff",
      "stroke-width": "20",
    ...
    if (this.cursorCircle)
      setStyles(this.cursorCircle, { transition: "transform 150ms ease-out" });
    this.clickRipple = createSVG<SVGCircleElement>(
      "circle",
      {
        cx: "16.5",
        cy: "16.5",
  ...
  public async moveTo(x: number, y: number, duration = 500): Promise<void> {
    if (!this.svgElement || (this.currentX === x && this.currentY === y))
      return;
  ...
  public async click(animationDuration = 300): Promise<void> {
    ...
    setTimeout(
      () => this.clickRipple && (this.clickRipple.style.opacity = "0"),
      animationDuration / 2
    );
    return new Promise((res) =>
      setTimeout(() => {
        if (this.cursorCircle) this.cursorCircle.style.transform = "scale(1)";
        if (this.clickRipple) this.clickRipple.style.transform = "scale(1)";
        res();
      }, animationDuration)
    );
  }
  ...
  public setVisibility(visible: boolean): void {
    ...
    this.svgElement.style.opacity = visible ? "1" : "0";
  }
  ...
  private updateCursorPosition(x: number, y: number): void {
    ...
    this.currentX = x;
    this.currentY = y;
    this.svgElement.style.left = `${x}px`;
    this.svgElement.style.top = `${y}px`;
  }
}
