...
import { TaskHistoryEntry } from "./TaskManager";
...
const KEYS = {
  CHATS: "superwizard_chats",
  CURRENT_CHAT_ID: "superwizard_current_chat_id",
  CHAT_PREFIX: "superwizard_chat_",
};
...
export const setTaskRunningState = async (
  ...
  await chromeStorageSet({
    ...
    taskTabId: isRunning ? targetTabId : null,
  });
  if (targetTabId) {
    try {
      ...
      // Silently ignore message failures
    }
  }
};
...
const storageOp = (operation: string, ...args: any[]): Promise<any> =>
  new Promise((resolve) => {
    try {
      if (!isChrome()) {
      ...
      const callback = (result?: any) => {
        if (chrome.runtime.lastError)
          console.error(
            `Chrome storage ${operation} error:`,
      ...
      switch (operation) {
        case "get":
          chrome.storage.local.get(args[0], callback);
          break;
        case "set":
          chrome.storage.local.set(args[0], () => callback());
          break;
        case "remove":
          chrome.storage.local.remove(args[0], () => callback());
          break;
        default:
...
const chromeStorageGet = (keys: string | string[]): Promise<any> =>
  storageOp("get", keys);
const chromeStorageSet = (items: Record<string, any>): Promise<void> =>
  storageOp("set", items);
const chromeStorageRemove = (keys: string | string[]): Promise<void> =>
  storageOp("remove", keys);

export const chromeStorage: StateStorage = {
  getItem: (name: string) =>
    chromeStorageGet(name).then((result) => result[name] || null),
...
export const createStorageSlice: MyStateCreator<StorageSlice> = (set, get) => ({
  ...
  actions: {
    ...
    createNewChat: async (title?: string) => {
      ...
      set((state) => {
        state.storage.chats.unshift(newChat);
        state.storage.currentChatId = chatId;
      });
    ...
    prepareNewChatSession: () => {
      set((state) => {
        state.storage.currentChatId = null;
      });
    ...
    deleteChat: async (chatId: string) => {
      set((state) => {
        state.storage.chats = state.storage.chats.filter(
          (chat) => chat.id !== chatId
        );
        if (state.storage.currentChatId === chatId) {
          const remaining = state.storage.chats;
          state.storage.currentChatId =
      ...
      await chromeStorageSet({
        [KEYS.CURRENT_CHAT_ID]: get().storage.currentChatId,
      });
    },
      ...
      set((state) => {
        ...
        if (chat) {
          ...
          const isFirstUserMsg =
            (message as any).role === "user" && chat.messages.length === 1;
    ...
    loadChatsFromStorage: async () => {
      ...
      try {
        ...
        set((state) => {
          state.storage.chats = chats;
          state.storage.currentChatId = currentChatId;
        });
      } catch (error) {
    ...
    saveChatToStorage: async (chat: Chat) => {
      try {
        await chromeStorageSet({ [`${KEYS.CHAT_PREFIX}${chat.id}`]: chat });
        ...
        if (!chatIds.includes(chat.id)) {
          ...
          await chromeStorageSet({ [KEYS.CHATS]: chatIds });
        }
      } catch (error) {
    ...
    searchChats: (query: string) => {
      ...
      return get().storage.chats.filter(
        (chat) =>
