 ...
 * stability.ts
 *
 * Advanced page stability checking with multiple verification methods
 */
...
async function pollForDOMStability(
  ...
  const { POLL_INTERVAL } = CONFIG.STABILITY;
  const startTime = Date.now();
...
export async function ensurePageStability(tabId: number): Promise<void> {
  const { MAX_WAIT_TIME, ADDITIONAL_BUFFER, FALLBACK_TIMEOUT } =
    CONFIG.STABILITY;
  ...
  try {
    ...
    await pollForDOMStability(tabId, CONFIG.STABILITY.DOM_STABILITY_TIMEOUT); // Give DOM time to stabilize
